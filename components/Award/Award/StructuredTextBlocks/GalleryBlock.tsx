...
const GalleryBlock: React.FC<Props> = ({ galleryRecords }) => {
  ...
  const images = galleryRecords.map((record: GalleryRecord, index: number) => (
    <div
      key={index}
      className={`relative h-72 w-full overflow-hidden rounded-xl lg:mx-6 lg:h-96 lg:w-1/2 ${
        index === page ? "active" : "hidden"
      }`}
    >
      <motion.div
        layout
      ...
      >
        <DatoImage
          ...
          data={record.imageGallery[0]?.responsiveImage} // Use the first image for now
        />
